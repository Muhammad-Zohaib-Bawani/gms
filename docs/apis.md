...
> All routes are under `api/v1`. Every endpoint returns an **`ApiResponse<T>`** envelope. Auth column: `Anon` = `[AllowAnonymous]`, `Auth` = authenticated only, `R:code` = `[HasPermission("code")]` (needs **Read** on that menu), `W:code` = `[HasPermission("code", Write)]` (needs **Write**). See `Backend/docs/role-access.md`. Request/Response DTOs are the classes in `Core/ViewModel/*`. "FE consumer" = frontend service/view. Fields not individually verified → see the DTO class. Endpoints marked **⚠ NC** are referenced by the frontend `endpoints.js` but were **not** seen in the controller route grep — **Needs confirmation**.
...
## Account Requests — **removed**

The self-service sign-up queue is gone: controller, service, view, DTOs, the
`AccountRequests` table and the `account-requests` menu row. The "NC" note that
used to sit here — *confirm where `RegisterAccountRequest` is posted* — was the
answer: nowhere. There was no public submit endpoint and no sign-up form, so the
queue could only ever be empty. Users are added by admin invite
...
## Role Access — `RoleAccessController` (`api/v1/role-access`)
Read/Write per **role**, per menu/submenu, on the `Permissions` + `RolePermissions`
tables. Replaces the old per-user `api/v1/user-access` and the old
`api/v1/permissions` catalogue.
| Method | Route | Auth | Request | FE consumer |
|---|---|---|---|---|
| GET | `/me` | Auth | — | `roleAccessService` / `AccessContext` (nav tree + flags) |
| GET | `/permissions` | R:`role-access` | — | admin tooling |
| GET | `/{roleId}` | R:`role-access` | — | `RoleAccessView` |
| PUT | `/{roleId}` | W:`role-access` | `SetRoleAccessRequest` (full replace) | `RoleAccessView` |

`/me` is open to every authenticated user: it only ever describes the caller's own
role. Its `permissions` are pruned — a parent appears because the role can read it
**or** because one of its submenus is readable, and in that case it carries only
the permitted children.

## Roles — `RolesController` (`api/v1/roles`)
| Method | Route | Auth | Request | FE |
|---|---|---|---|---|
| POST | `` | W:`role-access` | `CreateRoleRequest` | `roleService` |
| GET | `/{id}` | Auth | — | `roleService` |
| GET | `` | Auth | — | `roleService` |
| PUT | `/{id}` | W:`role-access` | `UpdateRoleRequest` | `roleService` |
| DELETE | `/{id}` | W:`role-access` | — | `roleService` |

Role reads are open to any signed-in user (the user editor and the account-request
approval screen both need the role roster for a dropdown, and the roster is only
names). `CreateRoleRequest`/`UpdateRoleRequest` no longer carry a permission list —
a role's menu access is set through `PUT /role-access/{roleId}`.

`PermissionsController` (`api/v1/permissions`) is **removed**: the permission
catalogue is the menu tree, and `GET /role-access/permissions` returns it.
...
> Travel reads are `R:services` and writes `W:services`. They used to be auth-only; if a seating or guest flow needs to write travel, that role needs Write on the Services menu.
...
Frontend has `organizationService` (`/v1/organizations`), `vehicleService` (`/v1/vehicles`), `OrganizationsView`, `VehiclesView`, and menus `organizations` / `vehicles`. Both controllers and entities now exist; this note is stale.
