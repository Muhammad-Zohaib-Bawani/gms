...
- `/` → `RequireAuth` (redirects to `/login` if not authed) → renders **`App`** (layout) with a nested `<Outlet/>`.
  ...
  - One route per module (from `MODULE_ROUTES`, keyed by permission code), each wrapped in `Guard code=…` → `canRead(code)`.
  - Dynamic: `lookups/:lookupKey` (→ `LookupsView`), `guests/:id` (→ `GuestDetailView`).
  - `*` → redirect to `/`.
- **Permission code → path** in `nav.js KEY_PATH`. Codes and paths must match `Permissions.Code`/`Permissions.Path` in the database (`Backend/docs/role-access-seed.sql` is the other half) — a component cannot come out of a row, so this table plus the router's key→component map is the code side of the contract. `pathForKey('lookup-<x>')` → `/lookups/<x>`. Paths are the ones already shipped; only `/user-access` was renamed, to `/role-access`.
...
## State management
- **Context:** `AuthContext` (session, user, `isAuthenticated`, `isDemo`, `signIn/out`, `enterDemo`), `AccessContext` (`menus` tree, `canRead(code)`, `canWrite(code)`), `EventsContext` (events list, `activeEvent`, `setActiveEventId`, persisted in `localStorage: gms-active-event`).
...
- **Services** (`src/api/services/`): `authService`, `userInviteService`, `dashboardService`, `eventService`, `guestService`, `invitationService`, `invitationTemplateService`, `travelService`, `locationService`, `lookupService`, `meetingService`, `nationalityService`, `notificationService`, `organizationService`, `roleService`, `roleAccessService`, `seatingService`, `supportChatService`, `uploadService`, `userAdminService`, `vehicleService`, `venueService`. Each is thin: build params → `apiClient.<verb>(ENDPOINTS.…)`.

## Auth (frontend)
- `tokenStore.js` persists only `{ accessToken, refreshToken }` in `localStorage` (`gms-auth`).
- `jwt.js` decodes the JWT client-side (no signature check) → user object + `read`/`write` permission-code lists (from the `read`/`write` claims).
- `AccessContext.canRead/canWrite(code)` gates routes and action buttons. Flags come off the token claims (instant, no request); the nav **tree** comes from `GET /role-access/me`, because claims are a flat list and cannot describe a hierarchy. "Explore demo" = in-memory pseudo-user for which every check passes (not persisted across reload).
...
| `RoleAccessView` | `/role-access` | Per-**role** Read/Write matrix over the menu tree | `roleAccessService`, `roleService` | tree comes from `Permissions` |
