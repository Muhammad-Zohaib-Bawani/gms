...
FormSchemaJson              RequiredGuestFieldsJson
        ...
        ─────────────────────
        GuestId, ServiceId
        Status               pending | completed   ← STORED; read as pending | confirmed
...
has at least one `completed` entry. Anything not yet reached is `locked` and cannot be
opened. Guests can always be created without services; the gate applies when services are
given, whenever that happens.

### The three statuses

`GuestServicePlanResponse.Slots[].Status` is one of:

| Status | Meaning |
|---|---|
| `pending` | May be worked on now. On a Fixed event, exactly one service is pending at a time — the first without a completed entry. |
| `confirmed` | Has at least one completed entry. (Stored as `completed`; see below.) |
| `locked` | Fixed events only — an earlier service in `SortOrder` is unfinished. `LockedReason` names it. |

All three are **derived per read** by `Core.Constants.ServiceFlow.Resolve`, a pure function
over `(isFixed, [(name, done)])`. Two consequences worth relying on:

- Completing a service promotes the next from `locked` to `pending` with nothing to write —
  the answer is simply recomputed. Likewise, clearing one re-locks what followed it.
- Changing a guest's service level, reordering a level's services, or switching the event
  between `fixed` and `flexible` re-answers the question on the next read. A stored status
  would need reconciling on all three; a derived one cannot drift. This is the same reasoning
  as **No status table** in §2, applied to the status value itself.

`locked` and `confirmed` exist **only on the wire**. The stored vocabulary is unchanged —
`pending | completed` — so no migration rewrites existing rows and pre-existing guests need
no backfill. `GuestServiceStatus.ToWire` maps stored → wire, and deliberately degrades an
unrecognised value to `pending` rather than `confirmed`, so a bad value never reads as work
that was never done. The frontend reads all of this through `lib/serviceStatus.js`, which
accepts both spellings.

`Slots[].IsUnlocked` is kept as `Status != "locked"` for clients that already gate on it;
`ServiceFlowTests` asserts the two can never disagree.

**Out-of-order completions.** A service that is `confirmed` while something earlier is still
`pending` is an invalid state the write path refuses to create — but legacy rows, or a level
reordered after the fact, can hold it. Such a slot reports `confirmed` (hiding real work
would be worse) while still carrying `IsUnlocked = false`, so no *new* entry can be written
to it until the chain ahead of it is repaired.

**Flexible has no lock at all.** `GuestServicePlanResponse.Slots[].IsUnlocked` is
unconditionally `true`, `Status` is never `locked`, and `IsRequired` is unconditionally
`false` when `GuestModel` is `flexible` (`ServiceFlow.Resolve`: `unlocked = !isFixed ||
blockedBy == null`). There is no "order" to speak of on a Flexible event — every assigned service can be
...
| Slot status | a booking row exists ⇒ confirmed (no draft state) | pending until an entry is completed |
