...
- Access control: users, roles, **role-level Read/Write per menu (Role Access)**, user invite accept. Menus and submenus are `Permissions` rows in the database; see `Backend/docs/role-access.md`.
...
- **Financials**, **Reports**, **Protocol** — views exist but have no menu row, no route and no controller. Their old action-code rows are retired with the rest; if they come back they need `Permissions` rows.
- The read/write collapse dropped some finer-grained gates that existed as their own `Permissions` rows: `Events.ManageStatus`/`ManageSessions`, `Guests.Import`/`Export`, `Travel.SyncHayya`, `Accreditation.Issue`/`Revoke`, `ServiceLevels.OverrideRules`. Each is now Write on its own menu. If any of these needs to be grantable separately again, it needs its own row or a third level.
...
### Medium priority
- **Accreditation:** issue/revoke are gated on Write of the **Guests** menu (they live on the guest record), not the Accreditation menu — decide the intended model. (API)
...
- **Read enforcement is now on.** The GETs on Guests/Events/Venue that used to have their permission attribute commented out carry `[HasPermission(code)]`. Cross-module dropdown reads that were deliberately open stay open: `/lookups/*`, `/organizations`, `/services`, `/roles`, `/vehicles`, `/events/{id}/fleet-providers` list reads.
