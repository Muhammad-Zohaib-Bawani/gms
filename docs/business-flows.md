...
|---|---|---|---|---|
| Auth & onboarding | `AuthView`, `UserInviteAcceptView` | `AuthController`, `UserInviteController`, `UsersController` | User, Role, Permission, RolePermission, UserRefreshToken, OtpVerification | Functional |
| Access control | `UsersView`, `RoleAccessView` | `UsersController`, `RoleAccessController`, `RolesController` | User, Role, Permission (one row per menu/submenu), RolePermission (CanRead/CanWrite) | Functional |
...
sequenceDiagram
  ...
  A->>S: LoginAsync
  S->>DB: user + role + role permissions
  ...
  U->>U: tokenStore.set; decode JWT; canRead/canWrite(code)
```
- **Access JWT claims:** identity (`uid/email/name/role/roleCode/roleId`) + **one `read` claim per permission code the role may read** and **one `write` claim per code it may write**. This is what both backend `[HasPermission]` and frontend `canRead`/`canWrite` read. Read never implies write; write does imply read.
...
- **Roles:** the roster is `Core/Constants/RoleDefinitions.cs`; what each role may see/change is `RolePermissions` rows, managed from the Role Access screen and seeded by `Backend/docs/role-access-seed.sql`. Policies are built on demand from the permission code (`PermissionPolicyProvider`).
- **Navigation:** built entirely from `GET /role-access/me`. A parent menu appears automatically when one of its submenus is readable, without needing an access record of its own, and then exposes only the permitted submenus. A parent with no reachable child and no access of its own is hidden.
- **Protected routes (FE):** `RequireAuth` → `/login`; per-route `Guard` checks `canRead(code)`, so typing the URL of a hidden page redirects. The endpoint's `[HasPermission]` enforces the same thing again — the frontend check only avoids showing a button that would 403. **Logout:** `/auth/logout` revokes the refresh token; frontend clears `tokenStore` and navigates to `/login`.
- **OTP:** email verification via `verify-otp` / `resend-otp` (also used by the VIP app's own OTP login).

## Admin invites a user
The only way a user gets created. (A self-service account-request queue used to
exist in code — controller, service, table, admin view — but nothing ever fed it:
there was no public submit endpoint and no sign-up form, so it could only ever be
