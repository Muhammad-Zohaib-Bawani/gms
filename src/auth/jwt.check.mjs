...
const live = token({ exp: now + 3600, uid: '7', email: 'a@b.c', read: ['services', 'guests'], write: ['guests'] });
const dead = token({ exp: now - 3600, uid: '7', email: 'a@b.c', read: 'services' });
...
// A single read/write claim arrives as a string, not an array — and an absent one
// must be an empty list, not undefined, or every canRead() call throws.
assert.deepEqual(userFromToken(dead).read, ['services']);
assert.deepEqual(userFromToken(dead).write, []);
assert.deepEqual(userFromToken(live).read, ['services', 'guests']);
assert.deepEqual(userFromToken(live).write, ['guests']);
