...
export default function SeatingView({ lang, activeEventId }) {
  ...
  // Assigning and unassigning a seat is the whole write surface of this screen.
  const canAssign = canWrite('seating');
  const isAr = lang === 'ar';
                      ...
                      {info ? (() => {
                        ...
                        return (
                          <span style={{
                            ...
                            color: cc || 'var(--accent)',
                            background: cc ? `${cc}1a` : 'rgba(0, 98, 123,0.08)',
                            border:`1px solid ${cc ? `${cc}4d` : 'rgba(0, 98, 123,0.2)'}`,
                          }}>
          ...
          <div className="card glass modal-solid" style={{ width:360, maxWidth:'92vw', padding:0 }}>
            ...
            <div style={{ padding:'16px 20px' }}>
              {!eventId ? (
                ...
                <div style={{ padding: '20px 0', textAlign: 'center', color: 'var(--ink-faint)', fontSize: 13 }}>
                  {isAr ? 'هذا المقعد فارغ.' : 'This seat is empty.'}
                </div>
              ) : (
                <>
