...
import { deleteUser, inviteUser, resendInvite, adminSetPassword, updateUser } from '../api/services/userAdminService';
...
function InviteUserModal({ open, onClose, roles, nationalities, driverTypes, activeEventId, onInvited }) {
  ...
  // Guests are not portal users — they come in through the guest flow, not an invite.
  const roleOpts = roles.filter((r) => r.code !== 'guest').map((r) => ({ value: r.id, label: r.name }));
...
// Three role-based tabs, one endpoint (GET /v1/users?roleCategory=…) — the old
// "Pending" tab is gone; it's a Status filter option now (see STATUS_OPTIONS)
// since "pending" was always just a status a user in any of these groups can
// be in, not a fourth kind of user.
const TABS = [
...
export default function UsersView({ activeEventId }) {
  ...
  // One endpoint fills it regardless of tab; `tab` just becomes the
  // roleCategory param (see load() below).
  ...
  // Filter panel — replaces the old standalone Pending tab (status: 'pending'
  // reproduces exactly what that tab used to show) plus a joined-date range
  // and a dedicated email filter, independent of the general name/email search.
  const [showFilters, setShowFilters] = useState(false);
  const [statusFilter, setStatusFilter] = useState('');
  const [emailFilter, setEmailFilter] = useState('');
  const [dateFrom, setDateFrom] = useState('');
  const [dateTo, setDateTo] = useState('');
  const activeFilterCount = [statusFilter, emailFilter, dateFrom, dateTo].filter(Boolean).length;
  ...
  // One Write flag covers create/update/delete — the API gates them the same way.
  ...
  // One request shape for all three tabs — `tab` maps straight onto
  // roleCategory, and the filter panel's fields ride along unconditionally
  // (undefined ones are dropped by axios, same as `search` already was).
  ...
  }), [pageIndex, pageSize, search, tab, statusFilter, emailFilter, dateFrom, dateTo]);

  const load = useCallback(async () => {
    if (isDemo) {
      // Demo mode has no drivers/guests seeded — only Platform Users shows rows.
      setRows(tab === 'platform' ? DEMO_USERS : []);
      setTotalRows(tab === 'platform' ? DEMO_USERS.length : 0);
      setLoading(false);
      return;
    }
    setLoading(true);
    try {
      const res = await apiClient.get(ENDPOINTS.users.base, { params: listParams });
      setRows(res?.items ?? []);
  ...
  }, [isDemo, tab, listParams]);

  useEffect(() => { load(); }, [load]);
  // Switching tabs or changing any filter starts over from page 1 — staying
  // on, say, page 3 of Platform Users and flipping to Drivers would otherwise
  // ask the server for a page that likely doesn't exist for the new list.
  useEffect(() => { setPageIndex(0); }, [tab, search, statusFilter, emailFilter, dateFrom, dateTo]);
    ...
    try {
      ...
      if (!isDemo) {
        const res = await apiClient.get(ENDPOINTS.users.base, {
          params: { ...listParams, pageNumber: 1, pageSize: Math.max(totalRows, 1) },
        });
        all = res?.items ?? [];
      }
      const isDriverTab = tab === 'drivers';
      const headers = [
        'First Name', 'Last Name', 'Email', 'Role', 'Status', 'Joined',
        ...(isDriverTab
          ? ['Driver Type', 'License Number', 'License Expiry', 'Nationality', 'Assigned Vehicle']
          : []),
      ];
      const csvRows = all.map((u) => [
        ...
        ...(isDriverTab ? [
          u.driverType || '', u.driverLicenseNumber || '', formatDate(u.driverLicenseExpiry),
          u.driverNationalityName || '', u.driverAssignedVehicle || '',
        ] : []),
      ]);
  ...
  const columns = useMemo(() => [
    ...
    // Driver-only columns — DriverProfile carries fields no other role has, so
    // they only make sense (and are only populated by the API) on this tab.
    ...(tab === 'drivers' ? [
      col.accessor('driverType', {
        header: 'Driver Type',
        size: 100,
        cell: ({ getValue }) => getValue()
          ? <span style={{ fontSize: 12 }}>{getValue()}</span>
          : <span style={{ color: 'var(--ink-faint)' }}>—</span>,
      }),
      col.accessor('driverLicenseNumber', {
        header: 'License No.',
        cell: ({ getValue }) => (
          <span style={{ fontSize: 12, fontFamily: 'var(--mono)' }}>{getValue() || '—'}</span>
        ),
      }),
      col.accessor('driverLicenseExpiry', {
        header: 'License Expiry',
        size: 120,
        cell: ({ getValue }) => (
          <span style={{ fontSize: 12, color: 'var(--ink-mute)' }}>{getValue() ? formatDate(getValue()) : '—'}</span>
        ),
      }),
      col.accessor('driverNationalityName', {
        header: 'Nationality',
        cell: ({ getValue }) => <span style={{ fontSize: 12 }}>{getValue() || '—'}</span>,
      }),
      col.accessor('driverAssignedVehicle', {
  ...
  ], [canCreate, canDelete, me?.id, resendingId, tab]);
  ...
  return (
    <>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">
            {loading ? 'Loading…' : `${totalRows} account${totalRows !== 1 ? 's' : ''}`}
          </div>
        </div>
      ...
      {showFilters && (
        <div className="card" style={{ padding: 14, marginBottom: 16, display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div style={{ minWidth: 160 }}>
            <label style={labelStyle}>Status</label>
            <Select value={statusFilter} onChange={(v) => setStatusFilter(v || '')} options={STATUS_OPTIONS} />
          </div>
          <div style={{ minWidth: 200 }}>
            <label style={labelStyle}>Email contains</label>
            <input
              style={inputStyle}
              value={emailFilter}
              onChange={(e) => setEmailFilter(e.target.value)}
              placeholder="e.g. gms.local"
            />
          </div>
          <div style={{ minWidth: 150 }}>
            <label style={labelStyle}>Joined from</label>
            <DateField value={dateFrom} onChange={(v) => setDateFrom(v || '')} clearable />
          </div>
          <div style={{ minWidth: 150 }}>
            <label style={labelStyle}>Joined to</label>
            <DateField value={dateTo} onChange={(v) => setDateTo(v || '')} clearable />
          </div>
          {activeFilterCount > 0 && (
            <button
              className="btn ghost"
              style={{ marginBottom: 1 }}
              onClick={() => { setStatusFilter(''); setEmailFilter(''); setDateFrom(''); setDateTo(''); }}
            >
              Clear filters
            </button>
          )}
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {/* Keyed by tab so switching between Platform/Guests/Drivers remounts
            the table instead of keeping its old page — the rows come from a
            fresh server query either way. */}
        <DataTable
          key={tab}
          columns={columns}
          data={rows}
