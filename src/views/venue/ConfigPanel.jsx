        ...
        {!selectedTable ? (
          ...
          >
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <input
                  ...
                  onChange={(e) =>
                    updateTable(selectedTable.id, { rotation: +e.target.value })
                  }
                />
                <input
                  type="number"
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {SWATCH_COLORS.map((sw) => (
                  <button
                    key={sw}
                    onClick={() => updateTable(selectedTable.id, { color: sw })}
                    style={{
                      width: 20,
                      height: 20,
                      borderRadius: "50%",
                ...
                {selectedTable.color && (
                  <button
                    ...
                    onClick={() =>
                      updateTable(selectedTable.id, { color: null })
                    }
                  >
                    {isAr ? "بلا لون" : "Clear"}
                  </button>
            ...
            {selectedTable.type === "round" && (
              <div>
                <label style={labelStyle}>
                  {t.seats} · {ad(selectedTable.seats)}
                </label>
                <input
                  type="range"
                  min={4}
                  ...
                  style={sliderStyle}
                  onChange={(e) =>
                    updateTable(selectedTable.id, { seats: +e.target.value })
                  }
                />
                <div
                  style={{
            ...
            {selectedTable.type === "rect" && (
              <div>
                <label style={labelStyle}>
                  {t.seatsPerSide} · {ad(selectedTable.seatsPerSide)}
                </label>
                <input
                  type="range"
                  min={2}
                  max={8}
                  value={selectedTable.seatsPerSide}
                  style={sliderStyle}
                  onChange={(e) =>
                    updateTable(selectedTable.id, {
                      seatsPerSide: +e.target.value,
                    })
                  }
                />
                <div
            ...
            {selectedTable.type === "stadium" && (
              <>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (
                        selectedSeat &&
                        selectedSeat.tableId === selectedTable.id &&
                        Math.floor(
                          selectedSeat.index / selectedTable.seatsPerRow,
                        ) >= newRows
                      )
                ...
                <div>
                  <label style={labelStyle}>
                    {t.seatsPerRow} · {ad(selectedTable.seatsPerRow)}
                  </label>
                  <input
                    type="range"
                    min={5}
                    max={20}
                    value={selectedTable.seatsPerRow}
                    style={sliderStyle}
                    onChange={(e) => {
                      ...
                      if (
                        selectedSeat &&
                        selectedSeat.tableId === selectedTable.id &&
                        selectedSeat.index % selectedTable.seatsPerRow >= newSpr
                      )
                        setSelectedSeat(null);
                    }}
                  />
                ...
                >
                  {ad(selectedTable.rows * selectedTable.seatsPerRow)}{" "}
                  {isAr ? "مقعد" : "seats total"}
                </div>
              </>
            )}
            ...
            {selectedTable.type === "stage" && (
              <>
                <div>
                  <label style={labelStyle}>
                    {t.stageWidth} · {ad(selectedTable.stageW || 220)}px
                  </label>
                  <input
                    type="range"
                    min={100}
                    max={400}
                    value={selectedTable.stageW || 220}
                    style={sliderStyle}
                    onChange={(e) =>
                      updateTable(selectedTable.id, { stageW: +e.target.value })
                    }
                  />
                </div>
                <div>
                  <label style={labelStyle}>
                    {t.stageDepth} · {ad(selectedTable.stageH || 80)}px
                  </label>
                  <input
                    type="range"
                    min={40}
                    max={160}
                    value={selectedTable.stageH || 80}
                    style={sliderStyle}
                    onChange={(e) =>
                      updateTable(selectedTable.id, { stageH: +e.target.value })
                    }
                  />
                </div>
                <div
            ...
            {!["round", "rect", "stadium", "stage"].includes(
              selectedTable.type,
            ) && (
              <>
                <div>
                  <label style={labelStyle}>
                    {t.areaWidth} · {ad(selectedTable.pitchW || 280)}px
                  </label>
                  <input
                    type="range"
                    min={120}
                    max={500}
                    value={selectedTable.pitchW || 280}
                    style={sliderStyle}
                    onChange={(e) =>
                      updateTable(selectedTable.id, { pitchW: +e.target.value })
                    }
                  />
                </div>
                <div>
                  <label style={labelStyle}>
                    {t.areaHeight} · {ad(selectedTable.pitchH || 140)}px
                  </label>
                  <input
                    type="range"
                    min={80}
                    max={300}
                    value={selectedTable.pitchH || 140}
                    style={sliderStyle}
                    onChange={(e) =>
                      updateTable(selectedTable.id, { pitchH: +e.target.value })
                    }
                  />
                </div>
                <div
                  style={{
                    fontSize: 11,
                    color: "rgba(90,191,110,0.85)",
                    background: "rgba(90,191,110,0.08)",
                    borderRadius: 6,
                    padding: "6px 10px",
                  }}
                >
            ...
            {hasSeats &&
              ...
              (() => {
                ...
                // The actual/position-derived code — never changes, even once a
                // placeholder is set, since it's what identifies the physical
                // seat later (e.g. when it's removed).
                ...
                return (
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const value = e.target.value
                          ...
                          .slice(0, 5); // Ensure max 5 characters

                        patchSeatMeta({ placeholder: value });
                      }}
                    />
                    ...
                    <input
                      ...
                      placeholder={
                        isAr
                          ? "اختياري — مثال: كرسي متحرك"
                          : "e.g. wheelchair access"
                      }
                      onChange={(e) =>
                        patchSeatMeta({ seatInfo: e.target.value })
                      }
                    />
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {SWATCH_COLORS.map((sw) => (
                        <button
                          key={sw}
                          onClick={() => patchSeatMeta({ color: sw })}
                          style={{
                            width: 18,
                            height: 18,
                            borderRadius: "50%",
            ...
            {hasSeats && (
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                ...
                >
                  ...
                  {deleteSeatMode ? t.exitDeleteMode : t.deleteSeats}
                </button>
                {selectedTable.removedSeats?.length > 0 && (
                  <button
                    ...
                    onClick={restoreSeats}
                  >
                    {isAr
                      ? `استعادة ${ad(selectedTable.removedSeats.length)} مقعد`
                      : `Restore ${selectedTable.removedSeats.length} seat(s)`}
                  </button>
                )}
                {deleteSeatMode && (
                  <div
