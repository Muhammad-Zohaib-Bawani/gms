...
import { isConfirmed, serviceStatusLabel } from '../lib/serviceStatus';

// A row with nothing filled into it yet — the guest's level assigns this service
// but no data has been entered. Cells render as dashes on their own
// (SectionCell); this only decides the status chip's wording.
const isEmptyEntry = (entry) =>
  !Object.values(entry?.values || {}).some((v) => v != null && String(v).trim() !== '');

const EMPTY_GUID = '00000000-0000-0000-0000-000000000000';

// False for a guest listed purely because the service is owed: there is no
// GuestServiceEntry behind them, so nothing to delete.
const hasEntry = (entry) => !!entry?.entryId && entry.entryId !== EMPTY_GUID;
      ...
      })),
      {
        ...
        cell: ({ row: { original: g } }) => (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {g.entries.map((e) => (
              <span key={e.entryId} className={`chip ${isConfirmed(e.status) ? 'confirmed' : 'pending'}`} style={{ fontSize: 10.5 }}>
                <span className="dot" />
                {/* Nothing filled in yet = Pending: the row exists because the
                    guest's level assigns this service. Draft is for one somebody
                    started and left half-done, which is a different follow-up. */}
                {isConfirmed(e.status) || isEmptyEntry(e)
                  ? serviceStatusLabel(e.status, isAr)
                  : (isAr ? 'مسودة' : 'Draft')}
              </span>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {g.entries.map((e) => (
              <ActionMenu
                key={e.entryId}
                items={
                  // A row that only records the service is owed has no entry to
                  // edit or delete — the menu opens the same service-specific
                  // modal as an Add instead.
                  !hasEntry(e)
                    ? [{ label: isAr ? 'إضافة' : 'Add', icon: 'plus', onClick: () => setBooking({ entry: e }) }]
                    : [
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {r.entries.map((e) => (
            <ActionMenu
              ...
              items={[
                { label: isAr ? 'تعديل' : 'Edit', icon: 'edit', onClick: () => setBooking({ entry: e }) },
                ...(canManage ? [{ label: isAr ? 'حذف' : 'Delete', icon: 'trash', danger: true, onClick: () => removeEntry(e) }] : []),
              ]}
            />
          ))}
        </div>
      ...
      <div>
        ...
        <Card>
          <EmptyState icon="star" title={isAr ? 'لا توجد خدمات إضافية' : 'No other services yet'}>
            {isAr
              ? 'الرحلات والإقامة والنقل موجودة في صفحة الخدمات. أنشئ خدمة جديدة من صفحة إدارة الخدمات لتظهر هنا تلقائياً.'
              : 'Flight, Accommodation and Transport live on Services. Create any other service from Manage Services and it appears here automatically.'}
          </EmptyState>
        </Card>
      </div>
    );
