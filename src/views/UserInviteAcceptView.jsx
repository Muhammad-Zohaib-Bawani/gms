...
import { getUserInvite, acceptUserInvite } from '../api/services/userInviteService';
...
export default function UserInviteAcceptView({ token }) {
  ...
  useEffect(() => {
    if (!token) { setError('This invite link is invalid.'); setLoading(false); return; }
    let cancelled = false;
    getUserInvite(token)
  ...
  const goToSignIn = () => { window.location.href = window.location.origin; };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {loading && (
          <div style={{ textAlign: 'center', color: 'var(--ink-mute)', padding: '30px 0' }}>Loading…</div>
        )}
        ...
        {!loading && !error && invite && (done || invite.alreadyAccepted) ? (
          <div style={{ textAlign: 'center', padding: '10px 0' }}>
            <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'rgba(90,191,110,0.15)', display: 'grid', placeItems: 'center', margin: '0 auto 16px' }}>
              <Icon name="check" size={22} style={{ color: '#5abf6e' }} />
            </div>
            <div style={{ fontSize: 15, marginBottom: 20 }}>
              {done ? 'Your account is set up. You can now sign in.' : 'This invite has already been accepted — you can sign in.'}
            </div>
            <button onClick={goToSignIn}
              style={{ width: '100%', padding: '13px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'var(--accent, #1aaec4)', color: '#fff', fontSize: 15, fontWeight: 600 }}>
              Go to Sign In
            </button>
          </div>
          ...
          <>
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              <div style={{ fontFamily: 'var(--serif, Georgia, serif)', fontSize: 24, fontWeight: 400, margin: '0 0 6px' }}>
                Welcome, {invite.firstName || 'there'}
              </div>
              <div style={{ fontSize: 13, color: 'var(--ink-mute)' }}>{invite.email}</div>
              {invite.roleName && (
                <div style={{
                  display: 'inline-block', marginTop: 10, fontSize: 11.5, padding: '3px 10px', borderRadius: 20,
                  background: 'rgba(26,174,196,0.10)', color: 'var(--accent)', border: '1px solid rgba(26,174,196,0.25)',
                }}>{invite.roleName}</div>
              )}
            </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                  ...
                  style={{ width: '100%', boxSizing: 'border-box', background: 'var(--surface-soft-3)', border: '1px solid var(--glass-border)', borderRadius: 10, padding: '11px 13px', color: 'var(--ink)', fontSize: 14 }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 10.5, color: 'var(--ink-mute)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 5 }}>
                  Confirm password
              ...
              <button type="submit" disabled={submitting}
                style={{ width: '100%', padding: '13px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'var(--accent, #1aaec4)', color: '#fff', fontSize: 15, fontWeight: 600, opacity: submitting ? 0.6 : 1 }}>
