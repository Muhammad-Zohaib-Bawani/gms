  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [ev, fl, acc, tr, st, nats, orgs, tmpls] = await Promise.all([
        ...
        // Each of the three is its own permission row and its endpoint is gated
        // on it, so asking without access is a guaranteed 403 — swallowed by the
        // catch, but still a request. Skipped outright instead.
        canRead('service-flight') ? getEventFlights(g.eventId).catch(() => null) : null,
        canRead('service-accommodation') ? getEventAccommodation(g.eventId).catch(() => null) : null,
        canRead('service-transport') ? getEventTransport(g.eventId).catch(() => null) : null,
