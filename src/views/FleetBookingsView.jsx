...
export default function FleetBookingsView({ lang, activeEventId }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // An empty grid and a failed fetch look the same, so say which it was.
      setRows([]);
      toast.fromError(err, isAr ? 'تعذّر تحميل الحجوزات' : 'Could not load bookings');
    }
    finally { setLoading(false); }
  }, [activeEventId, range.from, range.to, vehicleId, driverId, isAr]);
