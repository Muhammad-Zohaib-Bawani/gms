...
// Reads are open (the vehicle form needs the dropdown); writes need WRITE on
// the Fleet Providers menu.
export default function FleetProvidersView({ lang, activeEventId }) {
  const isAr = lang === 'ar';
  const { canWrite } = useAccess();
  const canManage = canWrite('fleet-providers');
  ...
  const load = useCallback(async () => {
    ...
    try { setRows((await getFleetProviders(activeEventId)) || []); }
      ...
      setRows([]);
      toast.fromError(err, isAr ? 'تعذّر تحميل المزودين' : 'Could not load providers');
    }
