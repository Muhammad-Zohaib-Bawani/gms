...
// server decides both — `status` (pending | confirmed | locked), `isUnlocked`
// and `lockedReason` come straight off the plan and are re-checked on save, so
// hiding the Add button here is presentation, not the enforcement.
...
// across two boxes.
const AD_SERVICE_CODE = 'arrival-departure';
...
function ConfirmedPill({ isAr }) {
  return (
    <span className="chip confirmed" style={{ fontSize: 10.5, flexShrink: 0 }}>
      <Icon name="check" size={10} /> {isAr ? 'مؤكد' : 'Confirmed'}
    </span>
  );
}
...
export default function GuestServicesPanel({
  ...
  function renderSlotCard(slot, override = {}) {
    // slot.status is the authority (pending | confirmed | locked); isUnlocked is
    // the same answer as a boolean and stays in step with it server-side.
    const locked = isLocked(slot.status) || !slot.isUnlocked;
    const confirmed = isConfirmed(slot.status);
      ...
      return (
        <GuestCard key={slot.serviceId} embedded={embedded}>
          <CardHeader icon={icon} title={title}>
            ...
            {confirmed && <ConfirmedPill isAr={isAr} />}
          </CardHeader>
