...
import { sourceRowText } from '../../components/ui/lookupSources';
import CompletedServiceCard from './cards/CompletedServiceCard';
...
// Each service is its own permission row (see Role Access), so a role can be
// allowed to book Transfers and not Flights. Write is the one that matters
// here — this component only ever exists to fill a service in.
const svcPermission = (code) => `service-${String(code || '').trim().toLowerCase()}`;

const CODE_ICON = {
  flight: 'flight',
  accommodation: 'hotel',
  transport: 'car',
  'arrival-departure': 'arrowsExchange',
};
...
export function validateServices(slots, pending, travel, isAr = false) {
  for (const slot of slots || []) {
    ...
    const touchedThisSession = !!pending?.[slot.serviceId]?.selected;
    if (!touchedThisSession && isConfirmed(slot.status)) continue;
...
function dynamicFieldFacts(form, values, lookups, isAr) {
  ...
  return Object.entries(values || {})
    ...
    .map(([k, v]) => {
      const f = byKey.get(k);
      const label = f ? ((isAr ? f.labelAr : null) || f.label || k) : k;
      let display = v;
      if (f?.type === 'checkbox') {
        display = (v === true || v === 'true') ? (isAr ? 'نعم' : 'Yes') : (isAr ? 'لا' : 'No');
      } else if (f?.type === 'select') {
        const opt = (f.options || []).find((o) => String(o.value) === String(v));
        if (opt) display = (isAr ? opt.labelAr : null) || opt.label || v;
      } else if (f?.type === 'lookup' && f.sourceKey) {
        const row = (lookups?.[f.sourceKey] || []).find((x) => String(x.id) === String(v));
        if (row) display = sourceRowText(f.sourceKey, row);
      }
...
export default function ServiceAccordion({
  ...
  const isSlotDone = (s) => {
    ...
    return !!p?.completed || isConfirmed(s.status) || !!(p?.extra?.length);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {visible.map((slot) => {
        ...
        // No write on this service = the row is readable but inert. Same
        // treatment as a completed slot, so the details already recorded still
        // show — only the inputs and the tick go away.
        const noAccess = !canWrite(svcPermission(slot.code));
        const viewOnly = noAccess || (lockOnDone && isConfirmed(slot.status) && !hasSessionState
          && !singleSlotId && !isAddingNew);
          
        const key = slot.isSystem ? TRAVEL_SECTION[slot.code] : null;
        const groups = !lockOnDone ? [] : [
          ...(slot.entries || []).map((e) => (slot.isSystem
            ? Object.entries(e.values || {}).filter(([, v]) => v != null && String(v).trim() !== '')
            : dynamicFieldFacts(slot.form, e.values, travelLookups, isAr))),
          ...(state?.extra || []).map((snap) => (slot.isSystem
            ? travelSectionFacts(key, snap, travelLookups, isAr)
            : dynamicFieldFacts(slot.form, snap.values, travelLookups, isAr))),
        ].filter((g) => g.length > 0);
        ...
        return (
          <div
            ...
            style={{
              ...
              overflow: 'hidden',
              boxShadow: expanded ? '0 0 0 3px rgba(0, 98, 123, 0.14)' : 'none',
          ...
          >
            <div style={{
              ...
              cursor: locked || noAccess ? 'not-allowed' : 'pointer',
            }}>
              ...
              >
                ...
                {done && !locked && !noAccess && !singleSlotId && allowAddAnother && !isAddingNew && (
                  <button
                    type="button"
                    className="icon-btn"
            ...
            {noAccess && (
              <div style={{
                ...
                fontSize: 11, color: 'var(--ink-faint)', padding: '0 12px 10px 41px',
              }}>
                <Icon name="shield" size={12} />
                {isAr
                  ? 'ليس لديك صلاحية تعديل هذه الخدمة. تواصل مع المسؤول.'
                  : "You don't have access to edit this service. Ask an administrator."}
              </div>
            )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: viewOnly ? 0 : 14 }}>
                    {groups.map((facts, gi) => (
                      <CompletedServiceCard
                        key={gi}
                        kind={slot.isSystem ? slot.code : 'dynamic'}
                        icon={slot.icon}
                        facts={facts}
                        isAr={isAr}
                        index={gi}
