...
import { PAGEABLE_SOURCES } from '../../../components/ui/lookupSources';
...
const FLIGHT_DATE_MARGIN_DAYS = 7;
// A single leg can't span more than this — departure to arrival, not the
// whole trip (each leg gets its own range field).
const FLIGHT_MAX_RANGE_DAYS = 3;
// A transfer is a single ride — pickup to dropoff can't run past a day.
export const TRANSPORT_MAX_SPAN_HOURS = 24;
...
export function FlightFields({ flight, setFlight, lookups = {}, isAr = false, eventMinDate, eventMaxDate }) {
  ...
  const flightMinDate = eventMinDate ? addDaysIso(eventMinDate, -FLIGHT_DATE_MARGIN_DAYS) : eventMinDate;
  const flightMaxDate = eventMaxDate ? addDaysIso(eventMaxDate, FLIGHT_DATE_MARGIN_DAYS) : eventMaxDate;
  ...
  // A return booking stores [outbound, inbound] (legTitle and the API's leg
  // indexing own that order), but an event guest flies IN to the event first and
  // OUT afterwards — so the inbound block is shown first. Render order only:
  // every write below still addresses its leg by the real stored index.
  const legOrder = flight.flightType === 'return' && legs.length > 1
    ? [1, 0]
    : legs.map((_, idx) => idx);
  ...
  // Route + times come as a pair from their combined controls, so both keys are
  // written in one patch (rather than two setLeg calls that would each rebuild
  // the legs array off stale state).
  const setLegRoute = (i, from, to) =>
    setFlight({ legs: legs.map((l, idx) => (idx === i ? { ...l, fromAirportId: from, toAirportId: to } : l)) });
  const setLegRange = (i, start, end) =>
    setFlight({ legs: legs.map((l, idx) => (idx === i ? { ...l, startTime: start, endTime: end } : l)) });
  ...
  return (
    <>
      ...
      {legOrder.map((i, pos) => {
        const leg = legs[i];
        if (!leg) return null;
        // Chained on DISPLAY order, not the stored index: each leg starts no
        // earlier than the one shown above it ends.
        const prev = pos > 0 ? legs[legOrder[pos - 1]] : null;
        return (
        <div key={i} style={{
          ...
          <div>
            ...
            {/* One field: pick the departure airport and the arrival field takes
                focus with its menu open — no second click. Airports are 1000+
                rows, so each half searches + scroll-loads server-side. */}
            <RoutePicker
              fromValue={leg.fromAirportId}
              toValue={leg.toAirportId}
              onChange={(from, to) => setLegRoute(i, from, to)}
              loadPage={PAGEABLE_SOURCES.airports.loadPage}
              resolveOption={PAGEABLE_SOURCES.airports.resolveOption}
              fromPlaceholder={isAr ? 'مطار المغادرة' : 'From airport'}
              toPlaceholder={isAr ? 'مطار الوصول' : 'To airport'}
              isAr={isAr}
            />
          </div>
          <div>
          ...
          <div>
            ...
            {/* Card in the form, calendar in a modal. Take-off/landing icons name
                the two ends without spending a line of label on each. */}
            <DateRangePicker
              mode="datetime"
              startValue={leg.startTime}
              endValue={leg.endTime}
              onChange={(start, end) => setLegRange(i, start, end)}
              minDate={prev?.endTime || prev?.startTime || flightMinDate}
              maxDate={flightMaxDate}
              maxRangeDays={FLIGHT_MAX_RANGE_DAYS}
              startLabel={isAr ? 'المغادرة' : 'Departure'}
              endLabel={isAr ? 'الوصول' : 'Arrival'}
              startIcon="planeTakeoff"
              endIcon="planeLanding"
              title={isAr ? 'تاريخ ووقت الرحلة' : 'Flight date & time'}
              isAr={isAr}
            />
          </div>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {section('accommodation', 'hotel', isAr ? 'الإقامة' : 'Accommodation', (<>
        ...
        <div>
          ...
          {!accommodationDatesReady ? (
            ...
            <>
              {/* The card now states the stay and its length, so the note below
                  carries only what it can't: how many rooms are left. */}
              <DateRangePicker
                mode="date"
                startValue={travel.accommodation.checkIn}
                endValue={travel.accommodation.checkOut}
              ...
              />
              {(() => {
                const { checkIn } = travel.accommodation;
                const left = checkIn && rooms.managed ? rooms.availableOn(checkIn) : null;
                if (left === null) return null;
                return (
                  <div style={{ fontSize: 11, color: 'var(--ink-faint)', marginTop: 6 }}>
                    {isAr
                      ? `${left} غرفة متاحة ليلة الوصول`
                      : `${left} room(s) left on the first night`}
                  </div>
                );
              })()}
            </>
      ...
      {section('transport', 'car', isAr ? 'النقل' : 'Transport', (<>
        ...
        {/* One route field: pick pickup, the dropoff field takes focus next. */}
        <div>
          <Label>{isAr ? 'المسار' : 'Route'} *</Label>
          <RoutePicker
            fromValue={travel.transport.pickupLocationId}
            toValue={travel.transport.dropoffLocationId}
            onChange={(from, to) => onChange((p) => ({
              ...p, transport: { ...p.transport, pickupLocationId: from, dropoffLocationId: to },
            }))}
            options={locationOpts}
            fromPlaceholder={isAr ? 'موقع الاستلام' : 'Pickup location'}
            toPlaceholder={isAr ? 'موقع التوصيل' : 'Dropoff location'}
            isAr={isAr}
          />
        </div>
        {/* Times come before the vehicle: the list below is filtered to cars free
            in that window, so picking one first would offer the whole fleet and
            then quietly narrow it. One range calendar + two times. */}
        <div>
          <Label>{isAr ? 'الاستلام والتوصيل' : 'Pickup & Dropoff'} *</Label>
          <DateRangePicker
            mode="datetime"
            startValue={travel.transport.pickupTime}
            endValue={travel.transport.dropoffTime}
            onChange={(start, end) => onChange((p) => ({
              ...p, transport: { ...p.transport, pickupTime: start, dropoffTime: end },
            }))}
            minDate={dateMinDate}
            maxDate={dateMaxDate}
