...
export default function GuestModal({
  open, onClose, guest, activeEventId, eventStartDate, eventEndDate,
  nationalities, templates, sessions, lang, onSaved,
}) {
  const isAr = lang === 'ar';
  const isEdit = !!guest;
  ...
  useEffect(() => {
    ...
    setRawTravel(null);
    if (guest?.id) {
      getGuestTravel(guest.id).then(setRawTravel).catch(() => setRawTravel(null));
    }
  }, [open, guest?.id]);

  useEffect(() => {
    setTravel(rawTravel ? hydrateTravel(rawTravel, travelLookups) : EMPTY_TRAVEL);
  ...
  const dateWindowMin = useMemo(() => addDaysIso(eventStartDate, -DATE_MARGIN_DAYS), [eventStartDate]);
  const dateWindowMax = useMemo(() => addDaysIso(eventEndDate, DATE_MARGIN_DAYS), [eventEndDate]);
  ...
  useEffect(() => {
    ...
    setForm((p) => {
      const outOfRange = (d) => !!d && ((dateWindowMin && d < dateWindowMin) || (dateWindowMax && d > dateWindowMax));
      ...
      return { ...p, arrivalDate: arrivalBad ? '' : p.arrivalDate, departureDate: departureBad ? '' : p.departureDate };
    });
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isEdit) {
        ...
        const created = await createGuest({ ...payload, eventId: activeEventId });
        guestId = created?.id;
      }
      ...
      if (guestId && anyTravelEnabled(travel)) {
        try {
          ...
          toast.error(isEdit
            ? (isAr ? 'تم تحديث الضيف لكن تعذّر حفظ بيانات السفر' : 'Guest updated, but travel details failed to save')
            : (isAr ? 'تم حفظ الضيف لكن تعذّر حفظ بيانات السفر' : 'Guest saved, but travel details failed to save'));
        }
      }
      ...
      toast.success(isEdit
        ? (isAr ? 'تم تحديث بيانات الضيف' : 'Guest updated successfully')
        : (templateId
          ? (isAr ? 'تمت إضافة الضيف وإرسال الدعوة' : 'Guest added & invitation sent')
          : (isAr ? 'تمت إضافة الضيف بنجاح' : 'Guest added successfully')));
    } catch (err) {
      toast.fromError(err, isEdit
        ? (isAr ? 'حدث خطأ أثناء تحديث الضيف' : 'Error updating guest')
        : (isAr ? 'حدث خطأ أثناء إضافة الضيف' : 'Error adding guest'));
    } finally {
      setSaving(false);
    }
  }
          ...
          <div style={{ padding: '18px 24px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexShrink: 0 }}>
            <div>
              <Dialog.Title style={{ margin: '0 0 12px', fontSize: 15, fontWeight: 600 }}>
                {isEdit ? (isAr ? 'تعديل الضيف' : 'Edit Guest') : (isAr ? 'ضيف جديد' : 'Add New Guest')}
                {isEdit && (
                  ...
                  return (
                    <React.Fragment key={i}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        <div style={{
                          width: 22, height: 22, borderRadius: '50%', display: 'grid', placeItems: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0,
            ...
            {step === 1 && (
              <>
                ...
                <div>
                  ...
                  <input placeholder={isAr ? 'مثال: وزارة الخارجية' : 'e.g. Ministry of Foreign Affairs'} value={form.organization} onChange={(e) => setF('organization', e.target.value)} style={inputStyle} />
                </div>
                <div>
                    ...
                    {sessions.map((s) => {
                      ...
                      return (
                        <div key={s.id} onClick={() => setGuestSessions((prev) => { const n = new Set(prev); n.has(s.id) ? n.delete(s.id) : n.add(s.id); return n; })}
                          style={{
                            padding: '8px 12px', borderRadius: 9, cursor: 'pointer', display: 'flex', alignItems: 'flex-start', gap: 10,
              ...
              <button className="btn primary" onClick={handleSave} disabled={saving}>
                ...
                {saving
                  ...
                  : isEdit
                    ...
                    : (templateId
                      ? (isAr ? 'إضافة وإرسال دعوة' : 'Add & Send Invite')
                      : (isAr ? 'إضافة الضيف' : 'Add Guest'))}
              </button>
            )}
          </div>
        </Dialog.Content>
