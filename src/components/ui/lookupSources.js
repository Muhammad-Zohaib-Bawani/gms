...
import { getAirportsPaged, getAirport } from '../../api/services/travelService';
...
export function sourceRowText(key, row) {
  if (!row) return '';
  const s = SOURCES[key];
  return (s ? s.text(row) : (row.name || row.id)) || '';
}

// Sources whose table is large enough to page/search on the server instead of
// loading the whole list into the dropdown. A field pointing at one of these
// renders as a lazy (scroll-to-load, search-as-you-type) select — see
// AsyncLookupSelect. Everything else keeps the one-shot loadLookupOptions above.
export const PAGEABLE_SOURCES = {
  airports: {
    loadPage: async ({ search, pageNumber, pageSize }) => {
      const res = await getAirportsPaged({ pageNumber, pageSize, search });
      const items = res?.items || [];
      return {
        options: items.map((a) => ({ value: String(a.id), label: sourceRowText('airports', a) })),
        hasMore: !!res?.hasNextPage,
      };
    },
    resolveOption: async (id) => {
      try {
        const a = await getAirport(id);
        return a ? { value: String(a.id), label: sourceRowText('airports', a) } : null;
      } catch { return null; }
    },
  },
};

export const isPageableSource = (key) => !!PAGEABLE_SOURCES[key];
