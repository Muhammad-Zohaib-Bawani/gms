...
import { LOOKUP_SOURCE_KEYS, lookupSourceLabel, loadLookupOptions, PAGEABLE_SOURCES, isPageableSource } from './lookupSources';
...
function LookupSelect({ field, value, onChange, isAr, ctx }) {
  ...
  React.useEffect(() => {
    // Pageable sources (e.g. airports) load lazily via AsyncLookupSelect below —
    // don't pull their whole list here.
    if (!field.sourceKey || isPageableSource(field.sourceKey)) return;
    let cancelled = false;
    setLoading(true);
  ...
  // Large tables (airports, …) — lazy, server-searched, scroll-paginated dropdown.
  if (isPageableSource(field.sourceKey)) {
    const cfg = PAGEABLE_SOURCES[field.sourceKey];
    return (
      <AsyncLookupSelect
        value={value}
