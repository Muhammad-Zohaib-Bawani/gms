...
export const ENDPOINTS = {
  auth: {
    login: '/v1/auth/login',
    // Trades a Microsoft Entra access token for a GMS token pair. Sent with the
    // ENTRA token as the Bearer header, so it must bypass the api client's
    // interceptors (see authService.loginWithMicrosoft).
    entraExchange: '/v1/auth/entra/exchange',
  ...
  // Role Access — read/write per role, per menu/submenu. Rows come from the
  // Permissions table, so `permissions` is the only place the nav tree is defined.
  roleAccess: {
    // The signed-in user's own nav tree + flags. Open to any authenticated user.
    me: '/v1/role-access/me',
    permissions: '/v1/role-access/permissions',
    byRole: (roleId) => `/v1/role-access/${roleId}`,
  },
  ...
  events: {
    base: '/v1/events',
    options: '/v1/events/options',
    optionById: (id) => `/v1/events/options/${id}`,
  ...
  lookups: {
    ...
    // Server-side paged + searched airports for the management table (1000+ rows).
    airportsPaged: '/v1/lookups/airports/paged',
    airportById: (id) => `/v1/lookups/airports/${id}`,
