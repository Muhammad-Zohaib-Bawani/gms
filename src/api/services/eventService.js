...
// Same shape as listEvents, but the endpoint is authentication-only — the event
// switcher runs for every role, including ones with no Events permission.
export function listEventOptions({ pageNumber = 1, pageSize = 100, search, status } = {}) {
  return apiClient.get(ENDPOINTS.events.options, {
    params: { pageNumber, pageSize, search: search || undefined, status: status || undefined },
  });
}

// Authentication-only, like listEventOptions: Accreditation, Guests, Seating and
// Services all read the active event and none of them imply Events permission.
export const getEvent = (id) => apiClient.get(ENDPOINTS.events.optionById(id));
