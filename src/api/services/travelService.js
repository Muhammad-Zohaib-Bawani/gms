...
// Server-side paged + searched airports for the management table. Returns a
// PaginatedResponse { items, totalCount, pageNumber, pageSize, ... }.
export const getAirportsPaged = ({ pageNumber = 1, pageSize = 15, search } = {}) =>
  apiClient.get(ENDPOINTS.lookups.airportsPaged, {
    params: { pageNumber, pageSize, searchTerm: search || undefined },
  });
...
export const updateAirport     = (id, body)      => apiClient.put(ENDPOINTS.lookups.airportById(id), body);
// One airport by id — lets a lazy dropdown show the label of the selected one.
export const getAirport        = (id)            => apiClient.get(ENDPOINTS.lookups.airportById(id));
