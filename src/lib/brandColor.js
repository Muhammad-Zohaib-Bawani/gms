...
const FALLBACK = { '--brand': 'hsl(278 99% 28%)', '--brand-2': 'hsl(278 78% 43%)', '--brand-deep': 'hsl(278 100% 18%)' };
...
export const brandTint = (alpha, token = '--brand-hsl') => {
  ...
  return channels ? `hsl(${channels} / ${alpha})` : `hsl(278 99% 28% / ${alpha})`;
};
...
export const brandHex = (token = '--brand-hsl') => {
  const m = (read(token) || '278 99% 28%').match(/(-?[\d.]+)\D+([\d.]+)%\s+([\d.]+)%/);
  const [h, s, l] = m ? [parseFloat(m[1]), parseFloat(m[2]) / 100, parseFloat(m[3]) / 100] : [278, 0.99, 0.28];
