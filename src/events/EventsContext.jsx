...
import { listEventOptions } from '../api/services/eventService';
...
const DEMO_EVENTS = [
  { id: 'doha-forum', key: 'doha-forum', title: 'Doha Forum', subtitle: '22nd Edition · 7–9 Dec', status: 'active', accent: '#00627b', secondary: '#158ba8', logoDark: 'assets/doha-forum-logo-white.png', logoLight: 'assets/doha-forum-logo.png' },
...
export function EventsProvider({ children }) {
  ...
  const reload = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const page = await listEventOptions({ pageSize: 100 });
      ...
      // Emptying the switcher silently made an outage look identical to an
      // account with no events.
      setEvents([]);
      toast.fromError(err, 'Could not load events');
    } finally {
      setLoading(false);
    }
